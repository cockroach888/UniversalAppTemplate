...
namespace DawnXZ.DawnGAT {
	...
	public partial class SplashScreen : Form {
		...
		public void ShowSplashScreen() {
			if (InvokeRequired) {
				BeginInvoke(new DawnXZ.FormUtility.Action(ShowSplashScreen));
				return;
			}
			this.Show();
		...
		public void CloseSplashScreen() {
			if (InvokeRequired) {
				BeginInvoke(new DawnXZ.FormUtility.Action(CloseSplashScreen));
				return;
			}
			FCloseFlag = true;
			this.Close();
		...
		public void MsgUpdate(string strText, MessageStatus msta) {
			if (InvokeRequired) {
				BeginInvoke(new DawnXZ.FormUtility.Action<string, MessageStatus>(MsgUpdate), new object[] { strText, msta });
				return;
			}
			switch (msta) {
